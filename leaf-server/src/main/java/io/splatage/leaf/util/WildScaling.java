...
public final class WildScaling {
    ...
    public static double getChunkDistanceFactor(
        final ChunkPos chunkPos,
        final double centerChunkX,
        final double centerChunkZ,
        final double maxChunkRadiusSq
    ) {
        ...
        final double dx = chunkCenterX - centerChunkX;
        final double dz = chunkCenterZ - centerChunkZ;
    ...
    public static double getBlockDistanceFactor(
        final BlockPos origin,
        final int centerX,
        final int centerZ,
        final int maxDistanceBlocks
    ) {
        if (maxDistanceBlocks <= 0) {
            return 0.0D;
        }

        final double dx = origin.getX() - centerX;
        final double dz = origin.getZ() - centerZ;
